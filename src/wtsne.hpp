...
inline std::vector<int> row_start_indices(const std::vector<int> &I,
                                               const size_t n_samples) {
  std::vector<int> row_start_idx(n_samples + 1);
  size_t i_idx = 0;
  row_start_idx[0] = 0;
...
std::vector<double> conditional_probabilities(const std::vector<int> &I,
...
#pragma omp parallel for schedule(static) num_threads(n_threads)
    ...
    const std::vector<int> row_start_idx = row_start_indices(I, n_samples);
...
#pragma omp parallel for schedule(static) num_threads(n_threads)
    for (int sample_idx = 0; sample_idx < n_samples; ++sample_idx) {
      ...
      for (int l = 0; l < n_steps; ++l) {
        sum_Pi = 0.0;
        for (int j = row_start_idx[sample_idx];
...
wtsne_init(const std::vector<int> &I, const std::vector<int> &J,
  ...
  const int nn = weights.size();
  const int ne = dists.size();
...
template <typename real_t>
std::vector<real_t>
wtsne_gpu(const std::vector<int> &I, const std::vector<int> &J,
          std::vector<real_t> &dists, std::vector<double> &weights,
          const real_t perplexity, const int maxIter, const int block_size,
          const int block_count, const int nRepuSamp, const real_t eta0,
